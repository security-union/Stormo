...
```swift
.package(url: "https://github.com/security-union/Stromo.git", from: "1.0.0")
```
...
**1. Add the package** to your app target, linking the `Stromo` and
`MPCCompat` products:
...
`NSBonjourServices` — Stromo uses the `_yourservice._udp` variant (Apple's
...
app keeps MPC's type names; the implementations come from MPCCompat. Stromo
deliberately does not publish `MC`-prefixed names, so the mapping lives in
*your* app:

```swift
//  MultipeerCompatAliases.swift — the entire MPC → Stromo mapping.
