...
/// The primary Stormo API: discovery, invitation, membership, and data
...
public actor PeerSession {
    ...
    /// latter is how StormoTestKit runs sessions without radios, QA-8).
    public init(
    ...
    public func invite(
        ...
        return try await withCheckedThrowingContinuation { continuation in
            guard inviteWaiters[peer.id] == nil else {
                continuation.resume(throwing: StormoError.invitationAlreadyPending(peer.id))
                return
            }
    ...
    public func send(
        ...
        if delivery == .datagram, payload.count > Delivery.maxDatagramPayload {
            throw StormoError.datagramTooLarge(
                bytes: payload.count, limit: Delivery.maxDatagramPayload)
        }
        guard !engine.members.isEmpty else { throw StormoError.peerUnreachable(identity.id) }
    ...
    public func sendResource(at url: URL, to peer: PeerID, name resourceName: String? = nil) async throws -> ResourceTransfer {
        guard engine.members.contains(peer), let connection = connections[peer] else {
            throw StormoError.peerUnreachable(peer)
        }
    ...
    public func leave() async {
        ...
        timers.removeAll()
        inviteWaiters.forEach { $0.value.resume(throwing: StormoError.peerUnreachable($0.key)) }
    ...
    private func deliver(_ event: ProtocolEngine.Event) {
        ...
        case .invitationFailed(let peer, let reason):
            ...
            switch reason {
            case .declined: error = StormoError.invitationDeclined
            case .timedOut: error = StormoError.invitationTimedOut
            case .connectionLost: error = StormoError.peerUnreachable(peer)
            }
    ...
    private static func receiveResource(
        ...
        guard let handle = try? FileHandle(forWritingTo: tempURL) else {
            continuation.yield(.failed(name: name, from: from, error: StormoError.resourceTransferIncomplete))
            return
        }
        ...
        if failure == nil, !progress.isCancelled, received == totalBytes {
            ...
            continuation.yield(.failed(
                name: name, from: from,
                error: failure ?? StormoError.resourceTransferIncomplete))
        }
    }
