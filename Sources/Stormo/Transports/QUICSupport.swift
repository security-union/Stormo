...
enum QUICError: Error, Sendable, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .connectionClosed: return "Stormo QUIC: connection closed"
        case .shortRead: return "Stormo QUIC: stream ended mid-frame"
        case .identityMismatch: return "Stormo QUIC: peer key hash does not match its certificate"
        case .tlsIdentityUnavailable(let reason):
            return "Stormo QUIC: no local TLS identity — \(reason)"
        case .malformedStreamHeader: return "Stormo QUIC: malformed stream header"
        case .listenerFailed(let reason): return "Stormo QUIC: listener failed — \(reason)"
        case .serviceResolutionFailed:
            return "Stormo QUIC: Bonjour service endpoint did not resolve"
        case .protocolVersionMismatch(let local, let remote):
            ...
            return "Stormo QUIC: protocol version mismatch — local \(local), peer \(remote); \(hint)"
        }
    }
}
...
enum PeerHello {
    ...
    static func encode(_ peer: PeerID, version: ProtocolVersion = .current) -> Data {
        ...
        let root = Stormo_Wire_PeerHello.createPeerHello(
            &fbb,
            peerOffset: info,
    ...
    static func decode(_ data: Data) -> Decoded? {
        ...
        guard
            let root: Stormo_Wire_PeerHello = try? getCheckedRoot(
                byteBuffer: &buffer,
...
enum QUICStreamHeaderCodec {
    ...
    static func encode(_ info: StreamHeaderInfo) -> Data {
        ...
        let root = Stormo_Wire_StreamHeader.createStreamHeader(
            &fbb,
    ...
    static func decode(_ data: Data) throws -> StreamHeaderInfo {
        var buffer = ByteBuffer(data: data)
        let root: Stormo_Wire_StreamHeader
        do {
    ...
    private static func wireKind(_ kind: StreamHeaderInfo.Kind) -> Stormo_Wire_StreamKind {
        switch kind {
    ...
    private static func infoKind(_ kind: Stormo_Wire_StreamKind) -> StreamHeaderInfo.Kind {
        switch kind {
