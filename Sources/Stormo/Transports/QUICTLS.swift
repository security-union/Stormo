...
enum QUICTLS {
    ...
    private static func createFileKeychain() throws -> (SecKeychain, URL) {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("Stormo-kc-\(UUID().uuidString)", isDirectory: true)
    ...
    /// Builds `NWParameters` for a QUIC endpoint: ALPN `Stormo/1`, local
    ...
    private static func installVerifyBlock(_ sec: sec_protocol_options_t, trust: TrustPolicy) {
        let queue = DispatchQueue(label: "dev.securityunion.Stormo.quic.verify")
