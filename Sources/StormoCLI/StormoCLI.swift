...
///
///     Stormo advertise [--service _pmdemo._udp] [--name A] [--meta k=v]...
///     Stormo browse    [--service _pmdemo._udp] [--timeout 30]
///     Stormo host      [--service ...] [--name A] [--once]
///     Stormo join      [--service ...] [--name B] [--peer A] [--send ping] [--linger]
///
...
struct StormoCLI {
    ...
    static func main() async {
        ...
        guard let command = arguments.first, ["advertise", "browse", "host", "join"].contains(command) else {
            print("usage: Stormo advertise|browse|host|join [--service TYPE] [--name NAME] [--peer NAME] [--send TEXT] [--meta k=v] [--timeout SECS] [--once]")
            exit(64)
        }
        ...
        log("Stormo \(command) — name=\(name) service=\(service) pid=\(getpid())")
        ...
        do {
            ...
            case "join":
                ...
                let member = try await session.invite(
                    target, context: Data("stormo-cli".utf8), timeout: min(timeout, 15))
